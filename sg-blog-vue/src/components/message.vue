			...
			<div class="tmsg-commentshow">
				<ul class="tmsg-commentlist">
					<li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
						<article class="">
                            <header>
                                <img  :src="item.avatar"  :onerror="item.avatar">
                                <div class="i-name">
                                    {{item.username}}
                                </div>
                                <!-- <div class="i-class">
						...
						<ul v-show="item.children" class="tmsg-commentlist" style="padding-left:60px;">
							<li class="tmsg-c-item" v-for="(citem,cindex) in item.children" :key="'citem'+cindex">
								<article class="">
									<header>
										<img :src="citem.avatar" :onerror="citem.avatar">
										<div class="i-name">
	...
	export default {
		...
		methods: { //事件处理器
			...
			sendMsg: function() { //留言
				...
				if (that.textarea) {
					that.sendTip = '咻~~';
					sendComment(that.type, that.aid, that.rootId, that.toCommentId, that.toCommentUserId, that
						.textarea).then((response) => {
						that.textarea = '';
						that.rootId = -1;
						that.toCommentId = -1;
